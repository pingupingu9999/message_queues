...
void internal_wait(){
  int pid_to_wait = pcb_running -> syscall_args[0];
  int* result = (int*) pcb_running -> syscall_args[1];
  // error conditions:
  // the process has no children
  if (pcb_running -> children.first == 0){
    pcb_running -> syscall_retvalue = DSOS_EWAIT;
    return;
  }  
  ...
  ListItem* aux = pcb_running -> children.first;
  while(aux){
  ...
  if (awaited_pcb && awaited_pcb -> status == Zombie) {
    ...
    List_detach(&pcb_running -> children, (ListItem*) awaited_pcb_ptr);
    PCBPtr_free(awaited_pcb_ptr);

    // remove he pc from zombie pool
    List_detach(&zombie_list, (ListItem*) awaited_pcb);
    pcb_running -> syscall_retvalue = awaited_pcb->pid;
    if (result)
  ...
  // pick the next
  PCB* next_pcb_running = (PCB*) List_detach(&ready_list, ready_list.first);
  pcb_running = next_pcb_running;
}