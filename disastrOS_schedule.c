...
// replaces the pcb_running process with the next one in the ready list
void internal_schedule() {
  if (pcb_running) {
    disastrOS_debug("PREEMPT - %d ->", pcb_running->pid);
   }
  // at least one process should be in the pcb_running queue
  ...
  if (ready_list.first){
    ...
    pcb_running -> status = Ready;
    List_insert(&ready_list, ready_list.last, (ListItem*) pcb_running);
    next_process -> status = Running;
    pcb_running = next_process;
  }
